...
void read1(char name[8], int32_t blockNum, char buf[1024]) {
...
void write1(char name[8], int32_t blockNum, char buf[1024]) {
    for(int i = 0; i < 16 ; i++){
    ...
    while(fgets(buf, 256, fp)) {
        if(index == 0) {
            ...
            if(strcmp(line[0],"C") == 0) {
                ...
                read1(line[1], atoi(line[2]), print_buf);
                printf("%s\n", print_buf);
                free(print_buf);
            } else if (strcmp(line[0],"W") == 0) {
                print_buf = (char *) calloc(1024, sizeof(char));
                write1(line[1], atoi(line[2]), print_buf);
                printf("%s\n", print_buf);
                free(print_buf);
            } else {
